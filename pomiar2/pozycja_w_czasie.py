...
if __name__ == "__main__":
    ...
    # df=df,
    # df_positions=df_positions,
    # func = lambda df: universal_position_estimator(df,method="DLS", state_obj=(df_positions["x"].iloc[0], df_positions["y"].iloc[0])),
    # window_width=WINDOW_WIDTH,
    # window_step=WINDOW_STEP,
    # output_dir="output_DLS2"
    # )

    # ekf = EKFLocalizer(initial_position=(df_positions["x"].iloc[0], df_positions["y"].iloc[0]))
    ...
    for window_width in test_windows:
        ...
        save_trajectory_plot(df, window_width, folder_path="wykresy2_DLS/", func = lambda df: universal_position_estimator(df, method="DLS", state_obj=(df_positions["x"].iloc[0], df_positions["y"].iloc[0])))
