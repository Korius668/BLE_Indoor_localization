...
def get_full_trajectory(df, window_width, window_step,func=least_square_estimation):
...
def generate_frame(df, df_positions, center_time, window_width, func=least_square_estimation):
    df_window = df[
...
def precompute_frames(df, df_positions,  window_width, window_step, func=least_square_estimation, output_dir="frames"):
    ...
    for i, t in enumerate(slider_values):
        ...
        frame, x_t, y_t, active_position, p_x, p_y = generate_frame(df, df_positions, center_time, window_width, func=func)
...
if __name__ == "__main__":
    ...
    # df=df,
    # df_positions=df_positions,
    # func = lambda df: universal_position_estimator(df,method="DLS", state_obj=dls, window_step=WINDOW_STEP),
    # window_width=WINDOW_WIDTH,
    # window_step=WINDOW_STEP,
    # output_dir="output_DLS2"
    # )

    # ekf = EKFLocalizer(initial_state=(df_positions["x"].iloc[0], df_positions["y"].iloc[0],0,0))
    ...
    for window_width in test_windows:
        ...
        # dls = DLSLocalizer(df_positions["x"].iloc[0], df_positions["y"].iloc[0])
        # save_trajectory_plot(df, window_width, folder_path="wykresy2_DLS/", func = lambda df: universal_position_estimator(df, method="DLS", state_obj=dls, window_step=WINDOW_STEP))
        for siga in sigma:
            ekf = EKFLocalizer(initial_state=(df_positions["x"].iloc[0], df_positions["y"].iloc[0],0,0), sigma_a=siga)
            save_trajectory_plot(df, window_width, folder_path="wykresy2_EKF/", filename=f"EKF_{siga}.png",func = lambda df: universal_position_estimator(df, method="EKF", state_obj=ekf, window_step=WINDOW_STEP))
