...
from pomiar1.least_square import distance_between_2_points
    ...
    for j in range(X.shape[0]):
        for k in range(X.shape[1]):
            for cord in beacons_coords:
                calculated_distances = distance_between_2_points(j,k,cord[0], cord[1])
...
if __name__ == "__main__":  
    ...
    for i, d in calc_data.items():
        ...
        total_count = 0
        ax = plot_area_of_function(X, Y, d=d)
